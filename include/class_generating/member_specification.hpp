	...
	namespace reflection
	{
		...
		struct has_required_options<util::member_specification<Options...>, util::member_specification<RequiredOptions...>, Only>
		{
			static constexpr bool value = 
				((Only && sizeof...(Options) == sizeof...(RequiredOptions)) ||
				(sizeof...(Options) >= sizeof...(RequiredOptions))) &&
					([]<util::fixed_string MethodOption>(util::member_specification<MethodOption>)
					{ return (std::is_same_v<util::member_specification<MethodOption>, util::member_specification<RequiredOptions>> || ...); }
				(util::member_specification<Options>{}) && ...);
		};

		template <bool Only, util::fixed_string ...MemberSpecification, typename Member>
		...
		{
			static constexpr bool value = (sizeof...(Options) <= sizeof...(RequiredOptions)) ||
				([]<util::fixed_string MethodOption>(util::member_specification<MethodOption>)
					{ return (!std::is_same_v<util::member_specification<MethodOption>, util::member_specification<RequiredOptions>> && ...); }
				(util::member_specification<Options>{}) && ...);
		};
